...
\\
The following explanation assumes on linearity of the input-output mapping.
...
Introducing an adversary input $\tilde{x}$ as the sum of the original input $x$ and the perturbation $\eta$, we can write the activation function for $\tilde{x}$ like follows:

\begin{equation}
	f(\tilde{x}) = w^Tx + w^T\eta
\end{equation}

Maximizing the change induced by $\eta$ in relation to its largest magnitude $\norm{\eta}_\infty$, it is optimal to set $\eta = sign(w)$.
The effect on the activation for $\tilde{x}$ can be calculated by $m*n$, where $m$ is the average magnitude of $w$, and $n$ the number of dimensions of $w$ and the input $x$.
...
This computely very cheap method for finding adversarial examples was developed by Goodfellow et al. in 2015, and relies on the assumption of linearity in a range around an input $x$.
Given model parameters $\theta$, an input $x$, the correct output $y$ and the model's loss function $J$, a perturbation $\eta$ can be calculated as follows:
